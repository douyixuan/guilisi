var EditableTable = function () {

    return {
        //main function to initiate the module
        init: function () {
            var op = null;
            function restoreRow(oTable, nRow) {
                var aData = oTable.fnGetData(nRow);
                var jqTds = $('>td', nRow);

                for (var i = 0, iLen = jqTds.length; i < iLen; i++) {
                    oTable.fnUpdate(aData[i], nRow, i, false);
                }
                oTable.fnDraw();
            }

            function editRow(oTable, nRow) {
                var aData = oTable.fnGetData(nRow);
                var jqTds = $('>td', nRow);
                jqTds[0].innerHTML = '<input type="text" class="form-control small" value="' + aData[0] + '">';
                jqTds[1].innerHTML = '<input type="text" class="form-control small" value="' + aData[1] + '">';
                jqTds[2].innerHTML = '<input type="text" class="form-control small" value="' + aData[2] + '">';
                jqTds[3].innerHTML = '<input type="text" class="form-control small" value="' + aData[3] + '">';
                jqTds[4].innerHTML = '<a class="edit" href="">保存</a>';
                jqTds[5].innerHTML = '<a class="cancel" href="">取消</a>';
            }

            function saveRow(oTable, nRow) {
                var jqInputs = $('input', nRow);
                oTable.fnUpdate(jqInputs[0].value, nRow, 0, false);
                oTable.fnUpdate(jqInputs[1].value, nRow, 1, false);
                oTable.fnUpdate(jqInputs[2].value, nRow, 2, false);
                oTable.fnUpdate(jqInputs[3].value, nRow, 3, false);
                oTable.fnUpdate('<a class="edit" href="">编辑</a>', nRow, 4, false);
                oTable.fnUpdate('<a class="delete" href="">删除</a>', nRow, 5, false);
                oTable.fnDraw();
            }

            function cancelEditRow(oTable, nRow) {
                var jqInputs = $('input', nRow);
                oTable.fnUpdate(jqInputs[0].value, nRow, 0, false);
                oTable.fnUpdate(jqInputs[1].value, nRow, 1, false);
                oTable.fnUpdate(jqInputs[2].value, nRow, 2, false);
                oTable.fnUpdate(jqInputs[3].value, nRow, 3, false);
                oTable.fnUpdate('<a class="edit" href="">编辑</a>', nRow, 4, false);
                oTable.fnDraw();
            }
            //save --  insert &  update
            function mysqlop(oTable, nRow, op) {
                //alert(op);
                var data = {};
                /*
                var jqInputs = $('>td', nRow);
                data["nama"]=jqInputs[1].value;
                data["coin"]=jqInputs[2].value;
                data["money"]=jqInputs[3].value;
                data["beizhu"]=jqInputs[0].value;
                data["operation"]=op;
                */
                var aData = oTable.fnGetData(nRow);
                data["nama"]=aData[1];
                data["coin"]=aData[2];
                data["money"]=aData[3];
                data["beizhu"]=aData[0];
                data["operation"]=op;

                $.ajax({
                    type: "POST",
                    url:"php/addinfo.php",
                    data:data,
                    success: function (htmltxt) {
                        if(htmltxt == 200){
                            alert("保存成功");
                        }
                        else if(htmltxt == 404){
                            alert("顾客名字已存在，请先查找再更新！");
                            oTable.fnDeleteRow(nRow);
                        }
                        else alert("保存失败！");
                    }
                });
            }
            // delete
            function mysqlop2(oTable, nRow, op) {
                var data = {};
                var aData = oTable.fnGetData(nRow);
                data["nama"]=aData[1];
                data["coin"]=aData[2];
                data["money"]=aData[3];
                data["beizhu"]=aData[0];
                data["operation"]=op;
                $.ajax({
                    type: "POST",
                    url:"php/addinfo.php",
                    data:data,
                    success: function (htmltxt) {
                        if(htmltxt == 200){
                            alert("已删除");
                        }
                        else alert("删除失败！");
                    }
                });
            }

            var oTable = $('#editable-sample').dataTable({
                "aLengthMenu": [
                    [10, 15, 20, -1],
                    [10, 15, 20, "全部"] // change per page values here
                ],
                // set the initial value
                "iDisplayLength": 10,
                "sDom": "<'row'<'col-lg-6'l><'col-lg-6'f>r>t<'row'<'col-lg-6'i><'col-lg-6'p>>",
                "sPaginationType": "bootstrap",
                "oLanguage": {
                    "sLengthMenu": "每页 _MENU_ 条数据",
                    "oPaginate": {
                        "sPrevious": "上一页",
                        "sNext": "下一页"
                    }
                },
                "aoColumnDefs": [{
                        'bSortable': false,
                        'aTargets': [0]
                    }
                ]
            });
            
            jQuery('#editable-sample_wrapper .dataTables_filter input').addClass("form-control medium"); // modify table search input
            jQuery('#editable-sample_wrapper .dataTables_length select').addClass("form-control xsmall"); // modify table per page dropdown

            var nEditing = null;

            $('#editable-sample_new').click(function (e) {
                e.preventDefault();
                var aiNew = oTable.fnAddData(['', '', '', '',
                        '<a class="edit" href="">编辑</a>','<a class="cancel" data-mode="new" href="">取消</a>'
                ]);
                var nRow = oTable.fnGetNodes(aiNew[0]);
                editRow(oTable, nRow);
                nEditing = nRow;
                // insert 
                op = "insert";
                // fail
                
            });

            $('#editable-sample a.delete').live('click', function (e) {
                e.preventDefault();
                if (confirm("确认删除?") == false) {
                    return;
                }
                var nRow = $(this).parents('tr')[0];
                
                //delete mysql
                op="delete";
                mysqlop2(oTable, nRow, op);
                oTable.fnDeleteRow(nRow);
                //alert("已删除!");
            });

            $('#editable-sample a.cancel').live('click', function (e) {
                e.preventDefault();
                if ($(this).attr("data-mode") == "new") {
                    var nRow = $(this).parents('tr')[0];
                    oTable.fnDeleteRow(nRow);
                    op = "delete";
                    mysqlop2(oTable, nRow, op);
                } else {
                    op = "cancel";
                    restoreRow(oTable, nEditing, op);
                    nEditing = null;
                }
            });

            $('#editable-sample a.edit').live('click', function (e) {
                e.preventDefault();
                /* Get the row as a parent of the link that was clicked on */
                var nRow = $(this).parents('tr')[0];
                if(op!="insert") op = "update";
                if (nEditing !== null && nEditing != nRow) {
                    /* Currently editing - but not this row - restore the old before continuing to edit mode */
                    op="restore";
                    restoreRow(oTable, nEditing);
                    //restoreRow(oTable, nEditing, op);
                    editRow(oTable, nRow);
                    nEditing = nRow;
                } else if (nEditing == nRow && this.innerHTML == "保存") {
                    /* Editing this row and want to save it */
                    saveRow(oTable, nEditing);
                    nEditing = null;
                    //alert("Updated! Do not forget to do some ajax to sync with backend :)");
                    //update mysql
                    mysqlop(oTable, nRow, op);
                } else {
                    /* No edit in progress - let's start one */
                    editRow(oTable, nRow);
                    nEditing = nRow;
                }
                op = null;
            });
        }

    };

}();